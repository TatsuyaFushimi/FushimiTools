...
## 複数ツールで同じOAuth `token.json` を共有する場合、狭いスコープの新規スクリプトが既存の広いスコープを上書きするリスク（視聴維持率チェック、2026-08-24）

### 問題

`動画編集ツール/再編集めも作成くん/視聴維持率チェック/retention_check.py` を実装する際、既存の
`業務ツール/Task_PJ/sync/` のGoogle OAuth基盤（`credentials.json`/`token.json`）を流用し、SCOPESに
`yt-analytics.readonly` を追加した。この際レビュアンの指摘で、以下のリスクが発覚した。

同一の`token.json`を複数のツールが共有している場合、片方のツール（新規スクリプト等）が自分のSCOPES定義だけで
トークンをrefreshすると、そのrefresh時に発行されるアクセストークンは**新規スクリプトのSCOPESに基づくもの**になる。
既存ツールが必要としていた広いスコープ（例：Task_PJ側が持つ別のGoogle APIスコープ）が、新規スクリプトの狭いSCOPES
定義でのrefreshによって実質的に上書き・縮小され、既存ツール側が後から権限不足エラーになる可能性がある。

### 対処

共有`token.json`を使う新規スクリプトを作る際は、以下のいずれかを必ず行う。

1. 新規スクリプトのSCOPESを、既存スクリプトのSCOPESと**完全一致**させる（既存の全スコープ＋今回追加分を含む形にする）
2. 完全一致させない場合は、共有token.jsonのrefreshタイミングでスコープが縮小される可能性がある旨を
   コードコメントで明確に注意書きし、影響範囲（既存のどのツールが広いスコープを必要としているか）を明記する

→ 同一OAuth認証基盤（`credentials.json`/`token.json`）を複数ツールで使い回す設計全般に適用できる教訓。
新しいスクリプトを既存の認証基盤に相乗りさせる際は、SCOPESの差分を必ず確認すること。

---

## Google Docsアップロード機能の実装で判明したDrive API・OAuthの落とし穴（2026-08-24）

### `drive.file`スコープではアプリが作成していない既存フォルダに書き込めない

`https://www.googleapis.com/auth/drive.file`スコープは「アプリ自身が作成/開いたファイルのみ」にアクセスを限定するOAuthスコープ。
既存の共有フォルダに新規ファイルを作成する用途には使えず、フォルダの所有者・管理者権限を持つユーザーであっても
`404 File not found`エラーになる（実機確認済み）。
→ 既存フォルダへの書き込みが必要な場合は、より広い`https://www.googleapis.com/auth/drive`スコープが必要。

### 共有ドライブ（Shared Drive）内のフォルダを操作する場合は`supportsAllDrives=True`が必須

Drive APIの`files().create()`・`files().get()`等の呼び出しに`supportsAllDrives=True`を付けないと、
共有ドライブ内のファイル・フォルダは（アクセス権があっても）`404 Not Found`になる。
`files().list()`で共有ドライブ内のファイルを一覧したい場合は`includeItemsFromAllDrives=True`も追加で必要。

### Google Workspaceの「管理者」権限は、個別のDriveファイル・フォルダへのアクセス権とは別物

ユーザーが「自分は管理者だ」と言っても、それは組織全体の管理者権限であり、特定のDriveフォルダへの共有・アクセス権が
自動的に付与されるわけではない。フォルダへのアクセスには、そのGoogleアカウントを明示的に共有設定
（またはリンク共有）に追加する必要がある。

### 共有ドライブでは共有ロール名が異なる（「コンテンツ管理者」等）

通常のMy Driveフォルダの共有ロール（閲覧者・コメント可・編集者）とは別に、共有ドライブのメンバー種別には
「コンテンツ管理者」等の名称がある。ユーザーから聞いた共有設定の言葉（例：「コンテンツ管理者にした」）から、
対象が共有ドライブかどうかを判断する手がかりになる。

### 複数スクリプトで同じOAuth `token.json`を共有する場合、`Credentials.from_authorized_user_file(path, scopes=SCOPES)`の`scopes`引数チェックは意味をなさない

`Credentials.from_authorized_user_file(path, scopes=SCOPES)`のように`scopes`引数を渡すと、ファイルに実際に
保存されている値を無視して、渡した引数がそのまま`creds.scopes`に入ってしまう。そのため「`creds.scopes`に必要な
スコープが含まれているか」というチェックは、この読み込み方法では常に真になり意味をなさない。
→ 正しくは、token.jsonをJSONとして直接読み込み、保存されている実際の`scopes`フィールドが、コード側で要求する
`SCOPES`を全て含んでいるか確認する。不足していれば`creds`を`None`にして強制的にブラウザでの再認証フローに
入れる、という実装パターンが有効（`refresh()`だけでは新しいスコープの同意は取れないため）。

---

## 未コミット変更を挟むと、コミットメッセージが実際の変更範囲を過小に説明する事故が起きる（Frame.io連携（開発用）進捗一覧シート連携、2026-08-25）

### 経緯

2日間かけて「進捗一覧シート連携」機能を実装・実機テストした際、1日目の作業の最後に実装した3つの変更（限定公開スタンプ検知時のグレーアウト機能・確認期日等のクリア処理・Slack subtypeフィルタの緩和）が未コミットのまま一晩残った。
翌日、別件の4件修正を行った際、これらの前日の未コミット分と一緒に1つのコミットにまとめてしまい、コミットメッセージには前日分の変更が一切書かれていない状態になった。
レビュアンの最終チェックで`git log -p`による実差分確認からこの不一致が発覚し、一度NG判定を受けた（「コミットメッセージが実際の変更範囲を過小に説明している」）。実害（バグ）は無かったが、記録の正確性という観点で問題があった。

### 教訓・対策

1. 作業の区切り（セッションの終わり等）で未コミットの変更を残した場合、次回作業開始時に必ず`git status`/`git diff`で「今から手を入れようとしている変更」と「既存の未コミット分」を区別する。両者を無自覚に一括コミットしない
2. 複数の変更をまとめてコミットする際は、コミットメッセージに実際の差分内容を漏れなく書き出す。「〇〇を修正」のような一部だけの要約で済ませない
3. レビュアン（最終チェック担当）は、コミットメッセージの自己申告を鵜呑みにせず、`git diff`／`git log -p`で実差分を確認する習慣を持つ。今回もこの確認によって不一致が発覚した
